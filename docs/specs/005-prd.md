...
- **In-app alert** when a next negative date exists in the horizon
...
**In scope:** See [002-phase-1-scope.md](./002-phase-1-scope.md) — 8 screens, full data model, projection engine, category budgets, CSV import, pt-BR + en.

**Out of scope (Phase 2):** Snapshots & variance, what-if simulation, AI insights, full Google Sheets import, PWA offline, multi-currency.
...
**Rationale:** Projection logic (especially credit cards and recurrence) is the highest-risk work. Pure JS engine + fixtures de-risk the product before persistence and React forms absorb complexity. See [006-project-structure.md](./006-project-structure.md) for folder layout and runtime flows.

---

## 7. Tech stack (locked — see [decisions/](../decisions/))

| Layer | Choice | ADR |
|---|---|---|
| Monorepo | **pnpm workspaces** | [ADR-001](../decisions/ADR-001-monorepo-react-vite.md) |
| UI framework | **React** + **Vite** | ADR-001 |
| Language | **TypeScript** (all packages) | [ADR-002](../decisions/ADR-002-typescript.md) |
| Styling | **CSS Modules** + Paper tokens (not Tailwind) | [ADR-003](../decisions/ADR-003-css-modules.md) |
| Client data | **TanStack Query** + `packages/db` repos | [ADR-004](../decisions/ADR-004-client-data-layer.md) |
| Routing | **TanStack Router** | [ADR-005](../decisions/ADR-005-tanstack-router.md) |
| Storage | **Dexie vs SQLite WASM** — spike in US-0.2 | [ADR-006](../decisions/ADR-006-client-storage.md) (proposed) |
| Component dev / tests | **Storybook** + **Vitest** | [ADR-007](../decisions/ADR-007-storybook-vitest.md) |
| Engine | **`packages/engine`** — pure `projectCashFlow()` | ADR-001, ADR-004 |
| i18n | react-i18next or similar | Open — pick at scaffold |

**US-0.1** acceptance criteria satisfied except storage backend (deferred to US-0.2 spike per ADR-006).
...
| **9** | [Snapshots](../user-stories/09-snapshots.md) | US-9.1 – US-9.2 | Phase 2 |
| **10** | [Settings & data](../user-stories/10-settings-data.md) | US-10.1 – US-10.3 | P0 |

**Total:** 44 user stories across 11 epics (Epic 9 deferred to Phase 2).
...
|---|---|---|---|
| 1 | Client storage: Dexie vs SQLite WASM | Dev | Open — [ADR-006](../decisions/ADR-006-client-storage.md); spike in US-0.2 |
| 2 | Monorepo layout detail | Dev | Draft — [006-project-structure.md](../specs/006-project-structure.md) |
| 3 | i18n library | Dev | Open — react-i18next likely |
| 4 | Import/restore from JSON export in Phase 1? | Product | Open — export is P0; import optional |
