(function () {
  ...
  function resolveDayContext(cell) {
    ...
    var label = row.querySelector(".month-row__label--sticky") || row.querySelector(".month-row__label");
    ...
    var nameEl = row.querySelector(".month-row__name");
    var monthLabel = nameEl
      ? nameEl.textContent.trim()
      : rowMonth !== undefined && rowMonth !== ""
        ? monthNames[Number.parseInt(rowMonth, 10)]
  ...
  function closePanel() {
    ...
    if (quickAddForm && quickAddToggle) {
      ...
      quickAddToggle.classList.remove("quick-add__toggle--hidden");
      var quickAddFooter = quickAddToggle.closest(".day-panel__quick-add");
      var dayPanel = quickAddToggle.closest(".day-panel");
      if (quickAddFooter) quickAddFooter.classList.remove("day-panel__quick-add--open");
      if (dayPanel) dayPanel.classList.remove("day-panel--quick-add-open");
    }
  }
  ...
  if (quickAddToggle && quickAddForm) {
    ...
    function setQuickAddOpen(isOpen) {
      quickAddForm.classList.toggle("quick-add__form--open", isOpen);
      quickAddToggle.hidden = isOpen;
      quickAddToggle.classList.toggle("quick-add__toggle--hidden", isOpen);
      if (quickAddFooter) {
        quickAddFooter.classList.toggle("day-panel__quick-add--open", isOpen);
      }
      if (dayPanel) {
        dayPanel.classList.toggle("day-panel--quick-add-open", isOpen);
      }
    }
